...
| Ver.2.0.0 | **重大架構重構** — 全面改用 Flutter 重寫；採用 `models / utils / services / widgets / pages` 分層架構；直接讀取 `client.realm`（路徑可由使用者選取並自動記憶）；新增 CSV 匯出匯入、下載佇列、批量下載功能 |
| Ver.1.2.0 | 匯出的資料改以圖譜資料夾建立時間排序 |
| Ver.1.1.0 | 移除執行程式時出現的黑窗、調整 UI 布局 |
| Ver.1.0.0 | 程式推出 |
